...
// NewKubeClusterCollection creates a [Collection] over the provided Kubernetes clusters.
...
func (c *kubeClusterCollection) WriteText(w io.Writer, verbose bool) error {
	...
	for _, cluster := range c.clusters {
		...
		rows = append(rows, []string{common.FormatResourceName(cluster, verbose), labels})
	}
...
func kubeClusterHandler() Handler {
	return Handler{
		...
		updateHandler: updateKubeCluster,
...
func updateKubeCluster(ctx context.Context, client *authclient.Client, raw services.UnknownResource, opts CreateOpts) error {
	cluster, err := services.UnmarshalKubeCluster(raw.Raw, services.DisallowUnknown())
	if err != nil {
		return trace.Wrap(err)
	}
	...
	clusters = FilterByNameOrDiscoveredName(clusters, cluster.GetName())
	if len(clusters) == 0 {
		return trace.NotFound("Kubernetes cluster %q not found", cluster.GetName())
	}
