...
func (c *AlertCommand) Initialize(app *kingpin.Application, _ *tctlcfg.GlobalCLIFlags, config *servicecfg.Config) {
	...
	c.alertCreate.Flag("id", "Optional alert ID. If not provided, a random UUID will be generated.").StringVar(&c.alertID)

	c.alertDelete = alert.Command("delete", "Delete cluster alerts.").Alias("rm")
...
func (c *AlertCommand) Create(ctx context.Context, client *authclient.Client) error {
	...
	alert, err := types.NewClusterAlert(id, c.message, types.WithAlertSeverity(sev))
	if err != nil {
		return trace.Wrap(err)
	}
...
func (c *AlertCommand) Delete(ctx context.Context, client *authclient.Client) error {
	if err := client.DeleteClusterAlerts(ctx, proto.DeleteClusterAlertRequest{
		AlertID: c.alertID,
	}); err != nil {
		return trace.Wrap(err)
	}
