...
<Checkpoint
  ...
  description="If the installation failed or teleport is not found, try these steps."
>

- If `teleport` is not found, ensure `/usr/local/bin` is included in your `PATH`.
...
- Check logs for startup errors:
  ```code
  $ sudo journalctl -u teleport --no-pager
  ```
- Ensure the host has outbound internet access during installation.
- If systemd is not available, use a [TAR archive installation](#downloading-packages-and-tar-archives) instead.
...
<Checkpoint
  ...
  description="If the agent is not running or failed to start, try these steps."
>
...
$ sudo journalctl -u teleport --no-pager
```
- Verify the join token is valid and unexpired.
- Ensure the agent can reach the Teleport Proxy Service (firewalls, security groups).
- Ensure system time is synchronized (for example, using `chronyd` or `systemd-timesyncd`).  
  Clock skew can prevent agents from joining. Look for "clock skew detected" or certificate 
  validity errors in the logs, and run `timedatectl timesync-status` to verify system time is 
  correct and NTP is active. The output `System clock synchronized: yes` indicates the system clock has 
  successfully synced with a time source.
- On SELinux systems, check for denied operations:
  ```code
  $ ausearch -m avc -ts recent
  ```
- If your environment uses outbound TLS inspection or proxies, ensure Teleport traffic is not intercepted.
...
<Checkpoint
  ...
  description="If the installation failed or packages were not found, try these steps."
>

- Verify `ID`, `VERSION_ID`, and `VERSION_CODENAME` are in `/etc/os-release`.
- For apt-based systems, ensure the GPG key exists in `/etc/apt/keyrings/`.
- Verify the correct `TELEPORT_CHANNEL` is configured.
- If `teleport` or `tctl` works without `sudo` but fails with it, ensure `/usr/local/bin` is included in sudo’s secure path.
...
- Re-download packages if checksum verification fails.
- Ensure the correct installation method was used for TAR installs. For static installations, check that you ran the 
  install script from the extracted directory (`sudo ./install`). This copies binaries to `/usr/local/bin/` and sets 
  up the systemd unit file. For managed updates, you would use `sudo teleport-update enable`.
- Resolve dependency issues using:
```code
  $ apt-get install -f
  $ yum localinstall
  $ dnf localinstall
```  
- After enabling managed updates:
```code
  $ teleport-update enable
  $ sudo systemctl start teleport
```
- Ensure the package architecture matches the downloaded package (amd64, arm64, etc.) in the output:
```code
  $ uname -m
```
- If `systemd` does not recognize the Teleport service, reload unit files with:
  ```code
