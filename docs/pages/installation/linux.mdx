...
- If the installation script fails, verify that `curl` is installed and your server has internet connectivity.
- For permission errors, ensure you are running the script with appropriate privileges (use `sudo` if needed).
- If systemd is not installed, you may need to use a TAR archive installation instead.
...
<Checkpoint
  title="Verify agent installation"
  description="Confirm that the Teleport Agent is installed and running."
>
...
Here are some troubleshooting tips:
- If the service is not running, start it with `sudo systemctl start teleport`.
- If the service fails to start, check the logs with `journalctl -u teleport`.
- Ensure the agent can reach your Teleport cluster over the network (check firewalls and security groups).
- Verify that the join token is valid and has not expired.
- For SELinux-enabled systems, check if SELinux is blocking Teleport with `ausearch -m avc -ts recent`.
...
2. Teleport maintains DEB and RPM package repositories for different Linux
...
<Checkpoint
  title="Verify package repository installation"
  description="Confirm that Teleport was installed successfully from the package repository."
>
...
Here are some troubleshooting tips:
- If the repository cannot be found, verify that for apt-based installations the `ID` and `VERSION_CODENAME` variables match your distribution, and for RPM-based installations the `ID` and `VERSION_ID` variables match your distribution.
- For GPG key errors, ensure you downloaded the key to the correct location (`/etc/apt/keyrings/` for apt).
- If packages are not found, check that `TELEPORT_CHANNEL` is set correctly and the repository URL is accessible.
- For unsupported distributions, try setting `ID` to a compatible value from the supported distributions table (e.g., use `centos` for RHEL derivatives).
...
2. Specify your system Teleport version and system architecture by assigning an
...
3. Run the following commands to download a Teleport package or archive, unpack
...
<Checkpoint
  title="Verify manual installation"
  description="Confirm that Teleport was installed successfully from the downloaded package or archive."
>
...
Here are some troubleshooting tips:
- If the checksum verification fails, re-download the package and SHA256 file as the download may have been corrupted.
- If `teleport` is not found after TAR installation, ensure you ran the `./install` script or added the binary location to your `PATH`.
- For dependency errors with DEB packages, run `sudo apt-get install -f` to resolve missing dependencies.
- For dependency errors with RPM packages, use `yum localinstall` or `dnf localinstall` instead of `rpm -i` to automatically resolve dependencies.
- Verify the `SYSTEM_ARCH` variable matches your system architecture (`uname -m` to check).
