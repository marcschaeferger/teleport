...
## Step 1/4. Install the Event Handler plugin
...
## Step 2/4. Configure the Event Handler plugin
...
### Define RBAC resources and issue credentials for the Event Handler role
...
(!docs/pages/includes/plugins/operator-crs-event-handler.mdx plugin="teleport-plugin-event-handler"!)
</TabItem>
</Tabs>

## Step 3/4. Install the Fluentd output plugin for Datadog
...
## Step 4/4. Run the Teleport Event Handler plugin
...
Update the configuration file as follows. If you are using the `teleport-plugin-event-handler`
Helm chart with Teleport Kubernetes Operator enabled, you can ignore this step, as we
replaced the `teleport` section with the `crd` and `tbot` sections.
