...
func NewHandler(cfg Config, opts ...HandlerOption) (*APIHandler, error) {
	...
	if automaticUpgrades(cfg.Modules.Features()) && h.cfg.AutomaticUpgradesChannels == nil {
...
func (h *Handler) getUserContext(w http.ResponseWriter, r *http.Request, p httprouter.Params, c *SessionContext, cluster reversetunnelclient.Cluster) (any, error) {
	...
	features := h.cfg.Modules.Features()
	entitlement := features.GetEntitlement(entitlements.AccessMonitoring)
	accessMonitoringEnabled := entitlement.Enabled && features.AccessMonitoringConfigured

	userContext, err := ui.NewUserContext(user, accessChecker.Roles(), *features.ToProto(), desktopRecordingEnabled, accessMonitoringEnabled)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (h *Handler) installer(w http.ResponseWriter, r *http.Request, p httprouter.Params) (any, error) {
	...
	if h.cfg.Modules.BuildType() == modules.BuildEnterprise || h.cfg.Modules.Features().Cloud {
		...
		if h.cfg.Modules.IsBoringBinary() {
