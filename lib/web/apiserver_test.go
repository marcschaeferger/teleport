...
func TestGetWebConfig_WithEntitlements(t *testing.T) {
	...
	testModules := &modulestest.Modules{
		...
		TestFeatures: modules.Features{
			Entitlements: map[entitlements.EntitlementKind]modules.EntitlementInfo{
				...
				entitlements.Desktop:            {Enabled: true},
	...
	err = json.Unmarshal([]byte(str[:len(str)-1]), &cfg)
	require.NoError(t, err)
	require.Empty(t, cmp.Diff(expectedCfg, cfg))
...
func newWebPack(t *testing.T, numProxies int, opts ...webPackOptions) *webPack {
	...
	modules := &fakeModules{Modules: options.modules}
	...
	for p := range numProxies {
		...
		proxies = append(proxies, createProxy(ctx, t, proxyID, node, server.TLS, hostSigners, clock, modules, options.proxyOptions...))
	}
...
func createProxy(ctx context.Context, t *testing.T, proxyID string, node *regular.Server, authServer *authtest.TLSServer,
	hostSigners []ssh.Signer, clock *clockwork.FakeClock, m *fakeModules, opts ...proxyOption,
) *testProxy {
	t.Helper()
	cfg := proxyConfig{}
	for _, opt := range opts {
